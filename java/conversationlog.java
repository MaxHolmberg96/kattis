...
//        fs.flush();
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (!users.containsKey(s[0])) {
                users.put(s[0], new HashMap<>());
            }
            for (int j = 1; j < s.length; j++) {
                set.add(s[j]);
                users.get(s[0]).put(s[j], users.get(s[0]).getOrDefault(s[j], 0) + 1);
            }
        }
        ArrayList<si> list = new ArrayList<>();
        for (String s : set) {
            int sum = 0;
            for (HashMap<String, Integer> mem : users.values()) {
            ...
            if (sum != -1) {
                list.add(new si(s, sum));
            }
        }
        list.sort(si::compare);
        if (list.size() == 0) {
    ...
    static class si {
        ...
        public si(String s, int i) {
            this.s = s;
            this.i = i;
        }
        
        public static int compare(si si1, si si2) {
            return Integer.compare(si2.i, si1.i) != 0 ? Integer.compare(si2.i, si1.i) : si1.s.compareTo(si2.s);
        }
    }
